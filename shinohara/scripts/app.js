...
function hydrateText() {
  ...
  if (media && data.hero.mediaUrl) {
    media.style.backgroundImage = `linear-gradient(120deg, rgba(26,46,64,.45), rgba(255,145,77,.16)), url("${data.hero.mediaUrl}")`;
...
function renderActivities() {
  ...
  root.innerHTML = data.activities.map((item, index) => {
    const href = index === 0 ? data.brand.openChatUrl : data.brand.lineUrl;
    return `
...
function renderJobFilters() {
  ...
  const audiences = [
    { id: "all", label: "すべて" },
    { id: "individual", label: "個人向け" },
    { id: "referral", label: "紹介案件" },
    { id: "corporate", label: "企業向け" }
  ];
  const categories = [...new Set(data.jobs.map((job) => job.category))];

  selectors.filters.innerHTML = `
    <div class="filter-group" aria-label="対象">
  ...
  selectors.jobs.innerHTML = jobs.map((job) => {
    ...
    <article class="job-card asp-card reveal">
      ...
      <div class="job-card__actions">
        ...
        <a href="contact.html?job=${encodeURIComponent(job.title)}" class="job-card__action" data-job-title="${escapeHtml(job.title)}">お問合せ</a>
      </div>
    </article>
  `;
  }).join("");
...
function openJobModal(jobId) {
  ...
  content.innerHTML = `
    ...
    <div class="job-detail-actions">
      <a class="button primary" href="contact.html?job=${encodeURIComponent(job.title)}">この案件についてお問合せ</a>
